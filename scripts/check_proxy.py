...
def socks4_latency(ip, port, timeout=FAST_TIMEOUT):
    ...
    try:
        ...
        req = struct.pack(
            "!BBH4sB",
            0x04, 0x01, target_port, socket.inet_aton(target_ip), 0x00
        )
        s.sendall(req)
        resp = s.recv(8)
...
def deep_check(proto, ip, port):
    ...
    for host, hport, path, use_ssl in apis:
        try:
            if proto == "https":
                # 改动点：用 httpx 支持 HTTPS proxy
                proxy_url = f"https://{ip}:{port}"
                try:
                    with httpx.Client(proxies=proxy_url, verify=False, timeout=DEEP_TIMEOUT) as client:
